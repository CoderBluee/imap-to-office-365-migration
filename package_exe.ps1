...
# Description: Automated end-to-end pipeline to compile Prism IMAP to Office 365 Migration Tool,
...
param (
    ...
    [switch]$AppImageOnly      # Pass -AppImageOnly to stop after creating Prism-IMAP-to-Office-365-Migration.exe
)
...
$AppFolder = Join-Path $AppImageDir "Prism-IMAP-to-Office-365-Migration"
...
Write-Host "  Prism IMAP to Office 365 Migration Tool - Full EXE & Installer Pipeline" -ForegroundColor Cyan
...
$JpackageArgs = @(
    "--type", "app-image",
    "--name", "Prism-IMAP-to-Office-365-Migration",
...
$ExePath = Join-Path $AppFolder "Prism-IMAP-to-Office-365-Migration.exe"
if (-not (Test-Path $ExePath)) {
    Write-Error "Prism-IMAP-to-Office-365-Migration.exe was not created at expected location: $ExePath"
}
...
& $AiPath /edit $AipFile /SetProperty ProductName="Prism IMAP to Office 365 Migration Tool"
...
& $AiPath /edit $AipFile /NewShortcut -name "Prism IMAP to Office 365 Migration Tool" -dir DesktopFolder -target "[APPDIR]Prism-IMAP-to-Office-365-Migration\Prism-IMAP-to-Office-365-Migration.exe" -icon "$LogoIcon"
& $AiPath /edit $AipFile /NewShortcut -name "Prism IMAP to Office 365 Migration Tool" -dir SHORTCUTDIR -target "[APPDIR]Prism-IMAP-to-Office-365-Migration\Prism-IMAP-to-Office-365-Migration.exe" -icon "$LogoIcon"
...
& $AiPath /edit $AipFile /SetPackageName "Prism-IMAP-to-Office-365-Migration-Setup.exe" -buildname DefaultBuild
...
# 10. Sync to Drive
$driveJarDir = "I:\My Drive\Akas\Jars\IMAP to Office 365 Migration"
if (-not (Test-Path $driveJarDir)) { New-Item -ItemType Directory -Path $driveJarDir -Force | Out-Null }
Copy-Item $ProtectedJar.FullName (Join-Path $driveJarDir "Prism-IMAP-to-Office-365-Migration.jar") -Force
...
Copy-Item $AipFile (Join-Path $destAdvance "Prism-IMAP-to-Office-365-Migration.aip") -Force
...
Write-Host "     -> Prism-IMAP-to-Office-365-Migration-Setup.exe" -ForegroundColor White
Write-Host "  3. Standalone Protected JAR: $driveJarDir" -ForegroundColor White
Write-Host "     -> Prism-IMAP-to-Office-365-Migration.jar" -ForegroundColor White
Write-Host "========================================================" -ForegroundColor Green
