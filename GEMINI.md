# Prism IMAP to Office 365 Migration — JavaFX Desktop Application (Modular Template)
...
- **Package fat JAR & Obfuscate**:
  ...
  - `target/imap-to-office-365-migration-1.0-SNAPSHOT-win-<yyyyMMdd-HHmm>.jar` (~110 MB)
  - `target/imap-to-office-365-migration-1.0-SNAPSHOT-win-<yyyyMMdd-HHmm>-protected.jar` (~103 MB)
- **Dependencies**:
...
|---|---|---|
| **Single-File Setup Installer** | Final installer EXE for end-users (Advanced Installer `ExeInside`, shortcuts, icons, splash) | `I:\My Drive\Akas\EXE\Prism-IMAP-to-Office-365-Migration-Setup.exe` (~135.4 MB) |
| **Advanced Installer Project** | `.aip` file source for setup configurations | `I:\My Drive\Akas\Package Maker\Advance Package\Prism-IMAP-to-Office-365-Migration.aip` |
| **Packaging Automation Script** | Reusable pipeline script synced to Drive | `I:\My Drive\Akas\Package Maker\Advance Package\package_exe_imap_to_office_365_migration.ps1` |
| **Standalone Protected JAR** | Deployed copy of latest obfuscated Java 21 JAR | `I:\My Drive\Akas\Jars\IMAP Backup\Prism-IMAP-to-Office-365-Migration.jar` (~103.65 MB) |
| **Local Standalone App-Image** | Portable runtime folder with launcher for local developer testing | `target/app-image/Prism-IMAP-to-Office-365-Migration/Prism-IMAP-to-Office-365-Migration.exe` (~583 KB launcher + bundled JRE) |

### Understanding the 2 Types of EXEs
1. **Portable Launcher EXE (`target/app-image/.../Prism-IMAP-to-Office-365-Migration.exe` - 583 KB)**:
...
2. **Single-File Setup Installer EXE (`I:\My Drive\Akas\EXE\Prism-IMAP-to-Office-365-Migration-Setup.exe` - 135.4 MB)**:
...
- **Report file**: `~/Documents/Prism IMAP to Office 365 Migration/testing_report.txt` (appended across runs; one session per run separated by `═══` banners).
